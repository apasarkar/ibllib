...
def load_spike_sorting_fast(
    ...
    """
    _logger.warning(
        'Deprecation warning: brainbox.io.one.load_spike_sorting_fast will be removed in future versions.'
    ...
    kwargs = dict(
        eid=eid, one=one, collection=collection, revision=revision, dataset_types=dataset_types, brain_regions=brain_regions
    )
...
def load_spike_sorting(
    eid, one=None, probe=None, dataset_types=None, spike_sorter=None, revision=None, brain_regions=None, return_collection=False
):
    """
    From an eid, loads spikes and clusters for all probes
    The following set of dataset types are loaded:
    ...
    """
    _logger.warning(
        'Deprecation warning: brainbox.io.one.load_spike_sorting will be removed in future versions.'
        'Use brainbox.io.one.SpikeSortingLoader instead'
    )
    collection = _collection_filter_from_args(probe, spike_sorter)
    _logger.debug(f'load spike sorting with collection filter {collection}')
    spikes, clusters = _load_spike_sorting(
        eid=eid,
        one=one,
...
def load_spike_sorting_with_channel(
    eid,
    one=None,
    probe=None,
    aligned=False,
    ...
    channels = load_channel_locations(eid, one=one, probe=probe, aligned=aligned, brain_atlas=brain_atlas)
    clusters = merge_clusters_channels(clusters, channels, keys_to_add_extra=None)
    if nested is False and len(spikes.keys()) == 1:
        k = list(spikes.keys())[0]
        channels = channels[k]
        ...
        **kwargs,
        ...
        try:
            self.files[obj] = self.one.load_object(
                self.eid, obj=obj, attribute=attributes.get(obj, None), collection=collection, download_only=True, **kwargs
            )
